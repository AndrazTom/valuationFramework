...
def resolve_company_identifier(
    ...
    *,
    ...
    try:
        ...
        return _build_resolution(
            input_value=identifier,
            identifier_kind=_normalize_identifier_kind(query, "auto", fallback_from_search=True),
            query_used=query,
            sec_company=sec_company,
            yahoo_quote=yahoo_quote.as_dict(),
        )
...
def _normalize_identifier_kind(
    ...
    requested_kind: str,
    *,
    fallback_from_search: bool = False,
) -> str:
    ...
    if upper.isdigit():
        return "cik"
    if fallback_from_search and _looks_like_isin(upper):
        return "isin"
    if fallback_from_search and _looks_like_cusip(upper):
        return "cusip"
    return "ticker"
